...
import { View, TouchableOpacity, Text, Dimensions, Animated, Platform } from 'react-native';
...
const tabBarWidth = width - 32;
  ...
  // iOS: dip slightly into the thin home-indicator zone (looks fine there).
  // Android: sit a small gap ABOVE the system inset so the floating pill never
  // overlaps the 3-button nav bar (or gesture pill) under edge-to-edge mode.
  const dynamicBottom = Platform.OS === 'ios'
    ? Math.max((insets.bottom || 0) - 20, 2)
    : (insets.bottom || 0) + 8;
